"""
Packaging Machine Service - Main Entry Point
"""
import sys
import os
import time
...
from packaging_machine import PackagingMachine
...
def publish_status():
    ...
    status_topic = get_machine_status_topic("packaging", machine.machine_id)
...
def publish_telemetry():
    ...
    telemetry_topic = get_machine_telemetry_topic("packaging", machine.machine_id)
...
def main():
    ...
    machine_id = MACHINE_ID or f"packaging-{int(time.time())}"
    logger.info(f"Starting Packaging Machine Service: {machine_id}")

    # Initialize machine
    machine = PackagingMachine(machine_id)
    machine.start()

    # Initialize MQTT client
    mqtt_client = MQTTClient(
        client_id=f"packaging-machine-{machine_id}",
        broker=MQTT_BROKER,
        port=MQTT_PORT,
    ...
    command_topic = get_machine_command_topic("packaging", machine_id)
    mqtt_client.subscribe(command_topic, handle_command)

    # Start MQTT loop
    mqtt_client.loop_start()

    logger.info(f"Packaging machine {machine_id} is running...")
    ...
    try:
        ...
        logger.info("Packaging machine service stopped")
