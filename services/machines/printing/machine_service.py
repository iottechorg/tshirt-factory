"""
Printing Machine Service - Main Entry Point
"""
import sys
import os
import time
...
from printing_machine import PrintingMachine
...
def publish_status():
    ...
    status_topic = get_machine_status_topic("printing", machine.machine_id)
...
def publish_telemetry():
    ...
    telemetry_topic = get_machine_telemetry_topic("printing", machine.machine_id)
...
def main():
    ...
    machine_id = MACHINE_ID or f"printing-{int(time.time())}"
    logger.info(f"Starting Printing Machine Service: {machine_id}")

    # Initialize machine
    machine = PrintingMachine(machine_id)
    machine.start()

    # Initialize MQTT client
    mqtt_client = MQTTClient(
        client_id=f"printing-machine-{machine_id}",
        broker=MQTT_BROKER,
        port=MQTT_PORT,
    ...
    command_topic = get_machine_command_topic("printing", machine_id)
    mqtt_client.subscribe(command_topic, handle_command)

    # Start MQTT loop
    mqtt_client.loop_start()

    logger.info(f"Printing machine {machine_id} is running...")
    ...
    try:
        ...
        logger.info("Printing machine service stopped")
