"""
Qualitycheck Machine Service - Main Entry Point
"""
import sys
import os
import time
...
def publish_status():
    ...
    status_topic = get_machine_status_topic("qualitycheck", machine.machine_id)
...
def publish_telemetry():
    ...
    telemetry_topic = get_machine_telemetry_topic("qualitycheck", machine.machine_id)
...
def main():
    ...
    machine_id = MACHINE_ID or f"qualitycheck-{int(time.time())}"
    ...
    mqtt_client = MQTTClient(
        client_id=f"qualitycheck-machine-{machine_id}",
        broker=MQTT_BROKER,
        port=MQTT_PORT,
    ...
    command_topic = get_machine_command_topic("qualitycheck", machine_id)
    mqtt_client.subscribe(command_topic, handle_command)

    # Start MQTT loop
    mqtt_client.loop_start()

    logger.info(f"Qualitycheck machine {machine_id} is running...")
    ...
    try:
        ...
        logger.info("Qualitycheck machine service stopped")
